...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $dsrQuery = "SELECT * FROM dinner_delivery_schedule_riders WHERE id = '$dsr_id' AND date = '$date'";
    $dsrRes = mysqli_query($connection, $dsrQuery);
    $dsrDetails = mysqli_fetch_assoc($dsrRes);

    // $dsrId = $dsrDetails['id'];

    $dsrdQuery = "SELECT dsrd.location as new_location, dsrd.*, c.* FROM dinner_delivery_schedule_riders_details dsrd JOIN customers c ON dsrd.cust_number = c.cust_number WHERE dsrd.delivery_schedule_riders_id = '$dsr_id' ORDER BY dsrd.sequence ASC";
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $dealQuerySchedule = "SELECT * FROM customers_dinner_deals WHERE cust_id = '$customerId' AND date = '$date' AND schedule_status = 'not-assigned'";
        ...
        $dealQuerySpecial = "SELECT * FROM customers_dinner_deals WHERE cust_id = '$customerId' AND date = '$date'";
...
<div class="container-fluid px-4">
    <h1 class="mt-4">Dinner Delivery Schedule - <?php echo $date; ?></h1>
    ...
    <?php if (!empty($dsrDetails) && !empty($dsrdDetails)) : ?>
        <form action="../process/dinner_update_schedule_process.php" method="POST" class="mt-4" id="orderForm">
    ...
    <?php else : ?>
        <div class="alert alert-success" role="alert">No <strong>customer</strong> found!</div>
    <?php endif; ?>
    <hr>
</div>
