...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $dealQuery = "SELECT * FROM customers_deals WHERE cust_id = '$customerId' AND date = '$nextDay' AND group_status = 'not-assigned'";
    ...
    // $nextDay = date('Y-m-d');
    // Fetch pending deals for this customer
    $dealQuerySchedule = "SELECT * FROM customers_deals WHERE cust_id = '$customerId' AND date = '$nextDay' AND schedule_status = 'not-assigned'";
...
<script>
    ...
    if (success === 'true') {
        ...
        setTimeout(function() {
            window.location.href = '../public/delivery_schedule.php?date=' + date;
        }, 2000);
    }
...
<div class="container-fluid px-4">
    <h1 class="mt-4">Delivery Grouping - <?php echo $nextDay; ?></h1>
    <ol class="breadcrumb mb-4">
        <li class="breadcrumb-item"><a href="index.php">Dashboard</a></li>
        <li class="breadcrumb-item active">Date Delivery Grouping</li>
    </ol>

    <form method="POST" action="../public/delivery_schedule.php" class="d-flex">
    ...
    <div class="row">
        ...
        $groupQuery = "SELECT g.id, ldgi.id as itemId, g.name, ldg.date, ldgi.cust_number 
                       FROM groups g 
                       JOIN lunch_delivery_groups ldg ON g.id = ldg.group_id 
                       JOIN lunch_delivery_group_items ldgi ON ldg.id = ldgi.delivery_group_id 
        ...
        foreach ($groups as $groupId => $group) {
            ...
            foreach ($group['customers'] as $customer) {
                echo '<form action="../process/remove_lunch_group_id_process.php" method="POST">';
    ...
    $sql = "SELECT
        ...
        FROM delivery_schedule_riders dsr 
        LEFT JOIN delivery_schedule_riders_details dsrd 
