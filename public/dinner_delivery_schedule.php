...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $dealQuery = "SELECT * FROM customers_dinner_deals WHERE cust_id = '$customerId' AND date = '$nextDay' AND group_status = 'not-assigned'";
    ...
    // $nextDay = date('Y-m-d');
    // Fetch pending deals for this customer
    $dealQuerySchedule = "SELECT * FROM customers_dinner_deals WHERE cust_id = '$customerId' AND date = '$nextDay' AND schedule_status = 'not-assigned'";
...
<script>
    ...
    if (success === 'true') {
        ...
        setTimeout(function() {
            window.location.href = '../public/dinner_delivery_schedule.php?date=' + date;
        }, 2000);
    }
...
<div class="container-fluid px-4">
    <h1 class="mt-4">Dinner Delivery Grouping - <?php echo $nextDay; ?></h1>
    <ol class="breadcrumb mb-4">
        <li class="breadcrumb-item"><a href="index.php">Dashboard</a></li>
        <li class="breadcrumb-item active">Date Delivery Grouping</li>
    </ol>

    <form method="POST" action="../public/dinner_delivery_schedule.php" class="d-flex">
    ...
    <?php if (!empty($customers)) : ?>
        ...
        <form action="../process/dinner_delivery_groups_process.php" method="POST" id="orderForm" onsubmit="return validateForm()">
    ...
    <div class="row">
        ...
        $groupQuery = "SELECT g.id, ldgi.id as itemId, g.name, ldg.date, ldgi.cust_number 
                       FROM groups g 
                       JOIN dinner_delivery_groups ldg ON g.id = ldg.group_id 
                       JOIN dinner_delivery_group_items ldgi ON ldg.id = ldgi.delivery_group_id 
        ...
        foreach ($groups as $groupId => $group) {
            ...
            foreach ($group['customers'] as $customer) {
                echo '<form action="../process/remove_dinner_group_id_process.php" method="POST">';
    ...
    <h1 class="mt-4">Dinner Delivery Schedule - <?php echo $nextDay; ?></h1>
    <ol class="breadcrumb mb-4">
        <li class="breadcrumb-item"><a href="index.php">Dashboard</a></li>
    ...
    <?php if (!empty($customersSchedule)) : ?>
        ...
        <hr>
        <form action="../process/dinner_delivery_schedule_process.php" method="POST" class="mt-4" id="orderForm">
            <input type="hidden" name="date" value="<?php echo $nextDay ?>">
    ...
    $sql = "SELECT
        ...
        FROM dinner_delivery_schedule_riders dsr 
        LEFT JOIN dinner_delivery_schedule_riders_details dsrd 
            ...
            <div class="col-md-6">
                <div class="card mb-4">
                    <div class="card-body">
                        ...
                        <div class="d-flex">
                            <form method="POST" action="../public/dinner_update_schedule.php" class="mt-3">
                            ...
                            <form method="POST" style="margin-left: 10px;" action="../process/dinner_approve_message_process.php" class="mt-3">
                                <input type="hidden" name="dsr_id" value="<?= $dsr_id ?>">
                                <?php foreach ($rider['details'] as $detail) : ?>
                                    <input type="hidden" name="dsrd_id[]" value="<?= $detail['dsrd_id'] ?>">
                                <?php endforeach; ?>
