...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        foreach ($removed as $remove) {
            ...
            $updateCustQuery = "UPDATE customers_dinner_deals SET schedule_status = 'not-assigned' WHERE cust_id = '$cust_id' AND date = '$date'";
        ...
        $deleteDsrQuery = "DELETE FROM dinner_delivery_schedule_riders WHERE id = '$dsr_id'";
        mysqli_query($connection, $deleteDsrQuery);

        // Delete from delivery_schedule_riders_details table
        $deleteDsrdQuery = "DELETE FROM dinner_delivery_schedule_riders_details WHERE delivery_schedule_riders_id = '$dsr_id'";
        mysqli_query($connection, $deleteDsrdQuery);

        // Insert into delivery_schedule table
        $scheduleQuery = "INSERT INTO dinner_delivery_schedule (date) VALUES ('$date')";
        ...
        $ridersQuery = "INSERT INTO dinner_delivery_schedule_riders (riders_id, delivery_schedule_id, date, total_distance, total_rider_cost, total_time, round_route) 
        ...
        foreach ($custNumbers as $index => $custNumber) {
            ...
            // Update the number
            $updateCustNumber = "UPDATE customers_dinner_deals SET schedule_status = 'assigned' WHERE cust_id = '$cust_id' AND date = '$date'";
        ...
        header("Location: ../public/dinner_delivery_schedule.php?success=true&date=$date");
        exit();
    } catch (Exception $e) {
        ...
        header("Location: ../public/dinner_delivery_schedule.php?success=false&date=$date");
        exit();
    }
} else {
    // Redirect if not POST request
    header("Location: ../public/dinner_delivery_schedule.php&date=$date");
    exit();
}
