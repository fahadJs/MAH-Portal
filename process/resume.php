...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $updateCustNumberB = "UPDATE customers_breakfast SET status = 'active' WHERE cust_id = '$cust_id'";
        if (!mysqli_query($connection, $updateCustNumberB)) {
            throw new Exception("Failed to update status: " . mysqli_error($connection));
        }

        $updateCustNumberD = "UPDATE customers_dinner SET status = 'active' WHERE cust_id = '$cust_id'";
