...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $insertGroupQuery = "INSERT INTO breakfast_delivery_groups (date, group_id) VALUES ('$selectedDate', '$groupId')";
        if (!mysqli_query($connection, $insertGroupQuery)) {
            throw new Exception("Failed to insert: " . mysqli_error($connection));
        }
        ...
        foreach ($selectedCustomers as $customerNumber) {
            $insertItemQuery = "INSERT INTO breakfast_delivery_group_items (delivery_group_id, cust_number) VALUES ('$deliveryGroupId', '$customerNumber')";
            if (!mysqli_query($connection, $insertItemQuery)) {
                throw new Exception("Failed to insert into: " . mysqli_error($connection));
            }
            ...
            // Update the number
            $updateCustNumber = "UPDATE customers_breakfast_deals SET group_status = 'assigned' WHERE cust_id = '$cust_id' AND date = '$selectedDate'";
        ...
        header("Location: ../public/breakfast_delivery_schedule.php?success=true&date=$selectedDate");
    } catch (Exception $e) {
        // Rollback the transaction in case of any error
        mysqli_rollback($connection);
        error_log($e->getMessage());
        header("Location: ../public/breakfast_delivery_schedule.php?success=false&date=$selectedDate");
    }
}
